...
def _extract_obs_image(content: dict) -> np.ndarray:
    """Extract the observation image from a raw_prompt content item.

    Handles both single-image and video (list of frames) inputs.
    For video, uses the last frame as the current observation.
    Converts CHW -> HWC if needed so make_query_panel receives HxWx3.
    """
    if "image" in content:
        arr = np.asarray(content["image"])
    elif "video" in content:
        # video is (num_frames, C, H, W) as a tensor/array; take the last frame
        arr = np.asarray(content["video"])[-1]
    else:
        raise KeyError(f"Content item has neither 'image' nor 'video' key: {list(content.keys())}")
...
def _compute_rollout_panels(batch: DataProto, max_images=32, config: Any = None) -> None:
    ...
    for idx in unique_indices:
        ...
        img = _extract_obs_image(content)
...
def compute_motion_rollout_panels(
    ...
    (ride_name, semantic_goal, panel_image, rm_initial_mean, rm_relative_mean)
    quintuples for logging. The last two fields are per-group means of the
    ``rm_initial_raw`` / ``rm_relative_raw`` reward components published by the
    ...
    has_rm_relative = "rm_relative_raw" in batch.non_tensor_batch
    ...
    for gid in selected_group_ids:
        ...
        rm_relative_mean: float | None = None
        if has_rm_initial:
            init_vals = [
                v
                for v in (
        ...
        if has_rm_relative:
            relative_vals = [
                v
                for v in (
                    score_value_at(batch.non_tensor_batch, "rm_relative_raw", int(i)) for i in group_idx
                )
                if v is not None
            ]
            if relative_vals:
                rm_relative_mean = float(np.mean(relative_vals))

        panel_row_list.append((ride_name, semantic_goal, panel_img, rm_initial_mean, rm_relative_mean))

    return panel_row_list

def compute_validation_rollout_panels(
    batch: DataProto,
    reward_scores,
    max_groups: int = 32,
    config: Any = None,
    ...
    Each panel image has:
      - A header line: "<sample_id> | goal: <language_command>"
      - Horizontally stacked annotated images (one per rollout in the group):
          red   = motion plan iteration 0
          green = motion plan iteration 1 (refined)
    ...
    rm_initial_mean, rm_relative_mean). The last two are per-group means of the
    ``rm_initial_raw`` / ``rm_relative_raw`` components published by the
    ``delta_initial`` reward path; they are ``None`` for other reward types.
    """
    from cotnav.utils.draw import draw_polyline, make_query_panel
    from cotnav.prompts.interface import parse_and_unify, OutputFormat
    ...
    critic_output_format = OutputFormat.VERDICT_V1
    if config is not None:
        cfg_fmt = config.actor_rollout_ref.get('critic_output_format', None)
    ...
    uids = np.asarray(batch.non_tensor_batch["uid"]).reshape(-1)
    reward_arr = np.asarray(reward_scores).reshape(-1)
    bsz = reward_arr.shape[0]
    assert bsz == uids.shape[0], "Batch size mismatch between reward_scores and uids"
    ...
    for gid in selected_group_ids:
        ...
        for idx in group_idx:
            ...
            img_annotated = img.copy()
            try:
                trace0 = parse_and_unify(motion_raw, OutputFormat.TRAJECTORY_V1)
                if trace0 is not None and "trajectory" in trace0.unified:
                    img_annotated = draw_polyline(
                        trace0.unified["trajectory"], img_annotated,
                        color=(255, 51, 51), line_thickness=2, dot_radius=3,
                    )
                trace1 = parse_and_unify(motion_refine_raw, OutputFormat.TRAJECTORY_V1)
                if trace1 is not None and "trajectory" in trace1.unified:
                    img_annotated = draw_polyline(
                        trace1.unified["trajectory"], img_annotated,
                        color=(51, 255, 51), line_thickness=2, dot_radius=3,
                    )
            except (ValueError, TypeError):
                pass
            ...
            if gt_trace:
                img_annotated = draw_polyline(
                    gt_trace, img_annotated,
                    color=(51, 255, 255), line_thickness=2, dot_radius=3,
                )
            ...
            if critique_raw.strip():
                try:
                    ...
                    reason = "no critique"
            q_reasons.append(reason)
            q_rewards.append(float(reward_arr[idx]))

        ride_name = _build_ride_name(batch, int(group_idx[0]))
        semantic_goal = extra[int(group_idx[0])].get("semantic_goal", "No goal provided")

        q_samples = [
            {
                "image": q_images[i],
                "reason": q_reasons[i],
                "hausdorff": q_rewards[i],
                "scores": {"reward": [q_rewards[i]]},
            }
            for i in range(len(q_images))
        ]
        panel_img = make_query_panel(
        ...
        panel_row_list.append((ride_name, semantic_goal, panel_img, mean_reward, rm_initial_mean, rm_relative_mean))
...
def compute_data_metrics(
    ...
    # surface per-group means of the raw ``rm_initial`` / ``rm_relative`` reward
    ...
    has_rm_components = any(
        k in batch.non_tensor_batch
        for k in (
            "rm_absolute_raw", "rm_relative_raw", "rm_initial_raw",
            "mdist_absolute_raw", "mdist_relative_raw", "mdist_initial_raw",
        )
    )
    ...
    if log_tables and len(score_keys) > 0:
        ...
        if has_rm_components:
            columns += ["rm_initial_mean", "rm_relative_mean"]
        table = wandb.Table(columns=columns)
        for (ride_name, semantic_goal, panel_img, rm_initial_mean, rm_relative_mean) in panel_items:
            row = [ride_name, semantic_goal, wandb.Image(panel_img)]
            if has_rm_components:
                row += [rm_initial_mean, rm_relative_mean]
            table.add_data(*row)
        metrics[_panel_table_key("visual/rollout_panels", step_tag)] = table
    ...
    if log_tables and "uid" in batch.non_tensor_batch:
        ...
        if val_panel_items:
            ...
            if has_rm_components:
                val_columns += ["rm_initial_mean", "rm_relative_mean"]
            val_table = wandb.Table(columns=val_columns)
            for (
                ride_name, semantic_goal, panel_img, mean_reward, rm_initial_mean, rm_relative_mean,
            ) in val_panel_items:
    ...
    for score_key in [
        ...
        'rm_relative_raw',
        'rm_initial_raw',
        # Raw mdist-branch scalars (only present when mdist is enabled).
        'mdist_absolute_raw',
        'mdist_relative_raw',
        'mdist_initial_raw',
    ]:
